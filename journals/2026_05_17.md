...
- base64 encoding #bash #encode
  collapsed:: true
	...
	- ```bash
	  ...
	  ```
- global cli #bash #env
	- edits the new path variable in .profile or .bash_profile
	  logseq.order-list-type:: number
		- ```bash
		  export PATH=$PATH":$HOME/bin"
		  ```
	- 2. create the directory "bin" under the home dir "~/bin"
	- 3. create the script under "bin", the FULL script file name <filename>(.<extension>), will be the global command line
